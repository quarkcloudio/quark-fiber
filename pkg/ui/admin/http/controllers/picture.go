...
func (p *Picture) GetLists(c *fiber.Ctx) error {
	...
	for k, v := range pictures {
		v["path"] = utils.GetPicture(v["path"].(string)) + "?timestamp=" + strconv.Itoa(int(time.Now().Unix()))
		pictures[k] = v
	}
...
func (p *Picture) LocalUploadFromBase64(c *fiber.Ctx) error {
	...
	uid := 0
	if value, ok := utils.User(c, "id").(int); ok {
		uid = value
	}
	...
	result := map[string]interface{}{
		"id":   id,
		"name": fileName,
		"url":  utils.GetPicture(filePath + fileName),
		"size": fileSize,
	}
...
func (p *Picture) LocalUpload(c *fiber.Ctx) error {
	...
	if len(picture) > 0 {
		result = map[string]interface{}{
			"id":   picture["id"],
			"name": picture["name"],
			"url":  utils.GetPicture(picture["path"].(string)),
			"size": picture["size"],
		}
	...
	result = map[string]interface{}{
		"id":   id,
		"name": fileName,
		"url":  utils.GetPicture(filePath + fileNewName),
		"size": fileSize,
	}
...
func (p *Picture) Download(c *fiber.Ctx) error {
	...
	return c.Redirect(utils.GetPicture(path))
}

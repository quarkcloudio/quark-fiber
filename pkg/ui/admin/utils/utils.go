...
// 获取当前登录用户信息
func User(c *fiber.Ctx, field string) interface{} {
	getToken := GetToken(c)
...
// 获取图片
func GetPicture(id interface{}) string {
	http, path := "", ""
	webSiteDomain := WebConfig("WEB_SITE_DOMAIN")

	if webSiteDomain != "" {
		if WebConfig("SSL_OPEN") == "1" {
			http = "https://"
		} else {
			http = "http://"
		}
...
// 获取文件
func GetFile(id interface{}) string {
	http, path := "", ""
	webSiteDomain := WebConfig("WEB_SITE_DOMAIN")
